...
class PredictionEngine {
    constructor() {
        ...
        // Performance tracking for dynamic learning
        this.performance = {};
        ...
        this.confidenceThreshold = 0.55; // 55% minimum
    }
    ...
    frequencyAnalysis() {
        ...
        const avg = recent.length / 16; // 16 possible numbers (3-18)
        const predictions = [];
    ...
    hotColdAnalysis() {
        ...
        // Predict hot numbers (appearing frequently)
    ...
    trendAnalysis() {
        ...
        const trend = avg - this.history.slice(-20, -10).reduce((acc, r) => acc + r.number, 0) / 10;
    ...
    bigSmallMomentumAnalysis() {
        ...
        // Predict continuation of momentum
        if (bigCount > smallCount * 1.5) {
            // Big momentum - predict big numbers
            ...
            // Small momentum - predict small numbers
    ...
    evenOddPatternAnalysis() {
        ...
        // Predict based on even/odd momentum
    ...
    gapAnalysis() {
        ...
        // Predict numbers based on average gap
    ...
    // Algorithm 9: Time of Day Pattern Analysis
    timeOfDayAnalysis() {
        // Requires metadata with period information
        ...
        this.metadata.forEach((entry, idx) => {
            if (entry.period) {
                try {
                    ...
                    if (period === currentPeriod && this.history[idx]) {
    ...
    // Algorithm 10: Hourly Pattern Analysis
    hourlyPatternAnalysis() {
        ...
        this.metadata.forEach((entry, idx) => {
            if (entry.period) {
                try {
                    const parsed = this.parsePeriod(entry.period);
                    if (parsed.hour === currentHour && this.history[idx]) {
                        const num = this.history[idx].number;
    ...
    // Algorithm 11: Time Window Analysis (±15 minutes)
    timeWindowAnalysis() {
        if (!this.metadata || this.metadata.length < 10) {
            return { predictions: [] };
        }
        ...
        // Find patterns within ±15 minute window
        const windowData = {};
        this.metadata.forEach((entry, idx) => {
            if (entry.period) {
                try {
                    ...
                    // Within same hour and ±15 minutes
                    if (hourDiff === 0 && minuteDiff <= 15 && this.history[idx]) {
                        const num = this.history[idx].number;
        ...
        return { predictions, window: `${currentHour}:${String(currentMinute).padStart(2, '0')} ±15min`, data: windowData };
    }
    ...
    getStatistics() {
        ...
        return {
            ...
            mostCommon: sortedByFreq[0] ? { number: parseInt(sortedByFreq[0][0]), count: sortedByFreq[0][1] } : null,
            leastCommon: sortedByFreq[sortedByFreq.length - 1] ? { number: parseInt(sortedByFreq[sortedByFreq.length - 1][0]), count: sortedByFreq[sortedByFreq.length - 1][1] } : null,
            frequency
        };
    }
    ...
    getAlgorithmPerformance() {
        ...
        for (const [name, algo] of Object.entries(this.algorithms)) {
            performance[name] = {
                name: algo.name,
                weight: algo.weight,
                accuracy: (algo.weight * 100).toFixed(1) // Simplified for display
            };
        }
        return performance;
    }
...
// Algorithm 12: Markov Chain Transition Analysis
markovChainAnalysis() {
    ...
    // Build transition matrix (1-step and 2-step)
    ...
    // Get last number and predict next
...
bayesianProbabilityEngine() {
    ...
    // Start with uniform prior (all numbers equally likely)
    const prior = {};
    for (let n = 3; n <= 18; n++) {
        prior[n] = 1 / 16;
    }

    // Evidence 1: Recent frequency
    ...
    // Evidence 2: Time-based patterns
    const now = new Date();
    const currentHour = now.getHours();
    const hourFreq = {};
    ...
    const totalHour = Object.values(hourFreq).reduce((a, b) => a + b, 0);

    for (let n = 3; n <= 18; n++) {
        let prob = prior[n];

        // P(num|recent) = P(recent|num) * P(num) / P(recent)
        if (recentFreq[n]) {
            prob *= (recentFreq[n] / totalRecent) * 2;
        }

        // P(num|hour) = P(hour|num) * P(num) / P(hour)
        if (hourFreq[n] && totalHour > 0) {
            prob *= (hourFreq[n] / totalHour) * 1.5;
        }
    ...
    Object.entries(posterior).forEach(([num, prob]) => {
        ...
        if (normalized > 0.08) { // Top predictions
            predictions.push({
                number: parseInt(num),
                confidence: normalized * 1.5 // Boost for Bayesian
            });
        }
    });
...
// Algorithm 14: Streak & Momentum Analysis
streakMomentumAnalysis() {
    ...
    for (let i = last10.length - 1; i >= 0; i--) {
        const entry = last10[i];

        if (entry.isBig) {
            currentBigStreak++;
            if (i === 0 || !last10[i - 1].isBig) break;
        } else {
            currentSmallStreak++;
            if (i === 0 || last10[i - 1].isBig) break;
        }
    ...
    streaks.big = currentBigStreak;
    streaks.small = currentSmallStreak;

    // Calculate momentum
    const bigMomentum = last10.filter(e => e.isBig).length / 10;
    const smallMomentum = 1 - bigMomentum;

    const predictions = [];

    // Strong Big momentum (>70%) or long Small drought
    if (bigMomentum > 0.7 || streaks.small > 3) {
        // Predict continuation or reversion
        const targetNumbers = bigMomentum > 0.7 ?
            [14, 15, 16, 17, 18] :  // Continue big
            [11, 12, 13];           // Revert to small-big

        targetNumbers.forEach(num => {
            predictions.push({
                number: num,
                confidence: bigMomentum > 0.7 ? bigMomentum : 0.7
            });
        });
    }

    // Strong Small momentum or long Big drought
    if (smallMomentum > 0.7 || streaks.big > 3) {
        const targetNumbers = smallMomentum > 0.7 ?
            [3, 4, 5, 6, 7] :       // Continue small
            [8, 9, 10];             // Revert to big-small

        targetNumbers.forEach(num => {
            predictions.push({
                number: num,
                confidence: smallMomentum > 0.7 ? smallMomentum : 0.7
            });
        });
    }

    return { predictions, streaks, momentum: { big: bigMomentum, small: smallMomentum } };
}
...
diceCorrelationAnalysis() {
    ...
    this.metadata.forEach(entry => {
        if (entry.dice && entry.dice.length === 3) {
            ...
            correlations.dice1_dice2[key12] = (correlations.dice1_dice2[key12] || 0) + 1;
            correlations.dice1_dice3[key13] = (correlations.dice1_dice3[key13] || 0) + 1;
            correlations.dice2_dice3[key23] = (correlations.dice2_dice3[key23] || 0) + 1;
    ...
    sorted.forEach(([combo, count]) => {
        ...
        if (frequency > 0.05) { // At least 5% occurrence
            predictions.push({
                number: sum,
...
multiLevelPatternRecognition() {
    ...
    const levels = {
        micro: this.history.slice(-5),    // Last 5
        meso: this.history.slice(-15),    // Last 15
        macro: this.history.slice(-50)    // Last 50
    };
    ...
    // Find numbers that appear frequently across multiple levels
    const predictions = [];
    for (let n = 3; n <= 18; n++) {
        ...
        // Strong pattern if consistent across levels
...
updateAlgorithmWeights() {
    Object.keys(this.algorithms).forEach(algoName => {
        const perf = this.performance[algoName];
        if (perf && perf.predictions > 10) {
            const accuracy = perf.correct / perf.predictions;

            // Adjust weight based on accuracy
            // Good: accuracy > 0.6 → increase weight
            // Bad: accuracy < 0.4 → decrease weight
            if (accuracy > 0.6) {
                this.algorithms[algoName].weight *= 1.1;
            } else if (accuracy < 0.4) {
                this.algorithms[algoName].weight *= 0.9;
            }

            // Cap weights between 0.5 and 2.0
            this.algorithms[algoName].weight = Math.max(0.5, Math.min(2.0, this.algorithms[algoName].weight));
...
recordPredictionOutcome(prediction, actualNumber, wasCorrect) {
    if (!prediction || !prediction.contributors) return;

    prediction.contributors.forEach(algoName => {
        if (this.performance[algoName]) {
            this.performance[algoName].predictions++;
            if (wasCorrect) {
                this.performance[algoName].correct++;
            }
        }
    });

    // Update weights every 20 predictions
    const totalPredictions = Object.values(this.performance)
        .reduce((sum, p) => sum + p.predictions, 0);

    if (totalPredictions % 20 === 0) {
...
getPerformanceSummary() {
    const summary = {};
    Object.entries(this.performance).forEach(([name, perf]) => {
        summary[name] = {
            ...perf,
            weight: this.algorithms[name].weight.toFixed(2),
            displayName: this.algorithms[name].name
        };
    });
    return summary;
}
